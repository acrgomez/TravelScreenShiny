...
cat.labels = c('detected: departure fever screen', "detected: departure risk screen", "detected: arrival fever screen",
               ...
               'missed: undetectable', 'missed: evaded screening')
...
get_frac_caught = function(tSinceExposed, ff, gg, R0, meanToAdmit, dscreen, ascreen, incubation.d, f.evaded){
    ...
    ## Outputs
    screen.passengers(tSinceExposed, del.d, ff, gg, sdep, sa, rd, ra, phi.d, incubation.d, pathogen, relative = 0, split1 = 2, arrival_screen = ascreen, departure_screen = dscreen, frac_evade = f.evaded)
}
# ## Test function
...
get_frac_caught_over_time = function(ff, gg, R0, meanToAdmit, ascreen, dscreen, incubation.d, f.evaded){
    arrive.times=seq(0,15,0.1)
    sapply(arrive.times, function(tt){get_frac_caught(tSinceExposed = tt, ff, gg, R0, meanToAdmit, dscreen, ascreen,incubation.d, f.evaded)}) %>% t() %>% as.data.frame() %>% mutate(
...
one_bootstrap = function(meanIncubate, meanToAdmit, R0, ff, gg, del.d, arrival_screen, departure_screen, f.evaded){
    ...
    outcomesBoth=sapply(infAge, FUN = function(x){screen.passengers(x, del.d, ff, gg, sdep, sa, rd, ra, 0, incubation.d, pathogen, relative = 0, split1 = 2, arrival_screen, departure_screen, frac_evade = f.evaded)})
    ...
    outcomesFever=sapply(infAge, FUN = function(x){screen.passengers(x, del.d, ff, 0, sdep, sa, rd, ra, phi.d, incubation.d, pathogen, relative = 0, split1 = 2, arrival_screen, departure_screen, frac_evade = f.evaded)})
    ...
    outcomesRisk=sapply(infAge, FUN = function(x){screen.passengers(x, del.d, 0, gg, sdep, sa, rd, ra, phi.d, incubation.d, pathogen, relative = 0, split1 = 2, arrival_screen, departure_screen, frac_evade = f.evaded)})
...
make_plots = function(meanIncubate, meanToAdmit, R0, ff, gg, flight.hrs, screenType){
    ...
    # Ribbon Plot
    # -------------------------------
    get_frac_caught_over_time(ff, gg, R0, meanToAdmit, arrival_screen, departure_screen, incubation.d, f.evaded = f.evaded) %>%
    ...
    full_join(filter(temp, minOrMax == 'Min'), filter(temp, minOrMax == 'Max'), by = c('days.since.exposed', 'outcome'), suffix = c('min', 'max'))%>%
        ...
        mutate(outcome = factor(outcome, levels =c('dFever', 'dRisk', 'aFever', 'aRisk', 'mb', 'mf', 'mr', 'nd', 'nde'), 
                                labels =(cat.labels))) -> rib
        ## Plot

    
    
    bootList <- replicate(n = nboot, one_bootstrap(meanIncubate, meanToAdmit, R0, ff, gg, del.d, arrival_screen, departure_screen, f.evaded = f.evaded)) 
    ...
    meanOutcomes %>%
        ...
        mutate(outcome = factor(name, levels = (c('d.fever', 'd.risk', 'a.fever', 'a.risk', 'm.b', 'm.f', 'm.r', 'nd', 'nde')),
                                labels =(cat.labels)),
               strategy = factor(strategy)) -> stackedB
    
    dashedLine = group_by(stackedB, strategy) %>%
      filter(name %in% c('m.b', 'm.f', 'm.r', 'nd', 'nde')) %>%
      group_by(strategy) %>%
    ...
    return(grid.arrange(stackedBars,ribbon) )
}
